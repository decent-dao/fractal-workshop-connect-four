...
# A script to set up delegating a list of addresses for governance for a given Token Voting DAO.
#
# Originally written as a way to set up for an on Chain Connect 4 demo for Eth Denver 2023.
#
...
#   {daoTokenAddress} - the address of the DAO's ERC20Votes token
...
def main():
    ...
    daoTokenAddress = setupFile.readline().strip()
    ...
    tokenContract = web3.eth.contract(daoTokenAddress, abi=ERC20_VOTES_ABI)
    ...
    for participantAddress in participantAddresses:
        ...
        try:
            sendErc20(web3, tokenContract, executionAccount.address, executionPrivateKey, account[0].address, participantVotingPower)
        ...
        # send the participant address Eth to play the game with
        try:
        ...
        # delegate the voting tokens to the participant from our new account
        try:
            delegateErc20(web3, tokenContract, account[0].address, account[1], participantAddress)
        except Exception as e:
            logFailure(e, failures, participantAddress)
            continue
...
def logFailure(exception, outFile, address):
    print(str(exception))
